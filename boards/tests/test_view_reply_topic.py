...
class ReplyTopicTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.username = 'john'
        self.password = '123'
        user = User.objects.create_user(username=self.username, first_name='james', last_name='brown', email='john@doe.com', password=self.password)
        self.topic = Topic.objects.create(subject='Hello, world', board=self.board, starter=user)
        Post.objects.create(message='Lorem ipsum dolor sit amet', topic=self.topic, created_by=user)
        self.url = reverse('boards:topic_reply', kwargs={'board_pk': self.board.pk, 'topic_pk': self.topic.pk})
